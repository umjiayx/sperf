function psma_partial_recon_bed1(patient_id, patient_name)
    %% Parameters
    nx = 128;
    ny = 128;
    nz = 79;
    ...
    foldername = ['./recon/partial_recon/psma_', patient_id, '_b1/'];
    ...
    if ~exist(strcat(foldername, filename), 'file')
        ...
        assert(size(proj1, 3) == 360, 'Error: Total number of views is not 120!!!');
        ...
        sg = sino_geom('par', 'nb', ig.nx, 'na', na/downfactor, 'orbit_start', 1.5+3*(mod-1), 'orbit', 360, 'dr', ig.dx);
        f.dir = test_dir;
