function psma_linear_recon_bed2(patient_id, patient_name)
    %% Parameters
    nx = 128;
    ny = 128;
    nz = 79;
    ...
    foldername = ['./recon/linear_recon/psma_', patient_id, '_b2/'];
    filename = 'linearrecon_79slices.fld';
    ...
    if ~exist(strcat(foldername, filename), 'file')
        ...
        if downfactor == 2 || downfactor == 4 || downfactor == 8
            for j = 2:1:downfactor
                for i = j:downfactor:size(yi_tot, 3)
                   ...
                   % periodic boundary conditions in the angular dimension!!!
        ...
        sg = sino_geom('par', 'nb', ig.nx, 'na', na, 'orbit_start', 178.5, 'orbit', -360, 'dr', ig.dx);
        f.dir = test_dir;
